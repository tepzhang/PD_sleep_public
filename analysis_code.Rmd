...
for (var in outcome_variables){
  ...
  # LME model: resting beta ~ NREM/REM duration
  ...
  # Effect size: within-person partial R² for REM_duration
  df_centered_dur = df %>%
    group_by(subject) %>%
    mutate(
      REM_duration_mean   = mean(REM_duration, na.rm = TRUE),
      REM_duration_within = REM_duration - REM_duration_mean
    ) %>% ungroup()
  m_full    = lmer(outcome_var ~ NREM_duration + REM_duration_mean + REM_duration_within + (1|subject/hemisphere),
                   REML = FALSE, data = df_centered_dur)
  m_reduced = lmer(outcome_var ~ NREM_duration + REM_duration_mean + (1|subject/hemisphere),
                   REML = FALSE, data = df_centered_dur)
  message("Effect size (within partial R²) — REM_duration: ",
          round((sigma(m_reduced)^2 - sigma(m_full)^2) / sigma(m_reduced)^2, 4))

  # LME model: resting beta ~ NREM/REM latency
  ...
  # Effect size: within-person partial R² for REM_latency
  df_centered_lat = df %>%
    group_by(subject) %>%
    mutate(
      REM_latency_mean   = mean(REM_latency, na.rm = TRUE),
      REM_latency_within = REM_latency - REM_latency_mean
    ) %>% ungroup()
  m_full    = lmer(outcome_var ~ NREM_latency + REM_latency_mean + REM_latency_within + (1|subject/hemisphere),
                   REML = FALSE, data = df_centered_lat)
  m_reduced = lmer(outcome_var ~ NREM_latency + REM_latency_mean + (1|subject/hemisphere),
                   REML = FALSE, data = df_centered_lat)
  message("Effect size (within partial R²) — REM_latency: ",
          round((sigma(m_reduced)^2 - sigma(m_full)^2) / sigma(m_reduced)^2, 4))
...
for (var in outcome_variables){
  ...
  # LME model: EP outcome ~ NREM/REM duration
  ...
  m_full    = lmer(outcome_var ~ NREM_duration + REM_duration_mean + REM_duration_within + (1|subject/hemisphere),
                   REML = FALSE, data = df_ep_centered_dur)
  m_reduced = lmer(outcome_var ~ NREM_duration + REM_duration_mean + (1|subject/hemisphere),
                   REML = FALSE, data = df_ep_centered_dur)
  message("Effect size (within partial R²) — REM_duration: ",
          round((sigma(m_reduced)^2 - sigma(m_full)^2) / sigma(m_reduced)^2, 4))

  # LME model: EP outcome ~ NREM/REM latency
  ...
  m_full    = lmer(outcome_var ~ NREM_latency + REM_latency_mean + REM_latency_within + (1|subject/hemisphere),
                   REML = FALSE, data = df_ep_centered_lat)
  m_reduced = lmer(outcome_var ~ NREM_latency + REM_latency_mean + (1|subject/hemisphere),
                   REML = FALSE, data = df_ep_centered_lat)
  message("Effect size (within partial R²) — REM_latency: ",
          round((sigma(m_reduced)^2 - sigma(m_full)^2) / sigma(m_reduced)^2, 4))
...
for (var in outcome_variables){
  ...
  # LME model: morning outcome ~ NREM/REM spectral power
  ...
  # Effect size: within-person partial R² for REM_delta
  df_centered_pow = df %>%
    group_by(subject) %>%
    mutate(
      REM_delta_mean   = mean(REM_delta, na.rm = TRUE),
      REM_delta_within = REM_delta - REM_delta_mean
    ) %>% ungroup()
  m_full    = lmer(outcome_var ~ NREM_beta + REM_beta + NREM_delta + REM_delta_mean + REM_delta_within + (1|subject/hemisphere),
                   REML = FALSE, data = df_centered_pow)
  m_reduced = lmer(outcome_var ~ NREM_beta + REM_beta + NREM_delta + REM_delta_mean + (1|subject/hemisphere),
                   REML = FALSE, data = df_centered_pow)
  message("Effect size (within partial R²) — REM_delta: ",
          round((sigma(m_reduced)^2 - sigma(m_full)^2) / sigma(m_reduced)^2, 4))
}
...
for (var in outcome_variables){
  ...
  df_power = df_overnight_morning_combined_lmer %>%
    rename(outcome_var = var) %>%
    filter(disease == "PD")
  # LME model: self-report alertness ~ NREM/REM spectral power
  ...
  # Effect size: within-person partial R² for REM_delta
  df_sr_centered_pow = df_power %>%
    group_by(subject) %>%
    mutate(
      REM_delta_mean   = mean(REM_delta, na.rm = TRUE),
      REM_delta_within = REM_delta - REM_delta_mean
    ) %>% ungroup()
  m_full    = lmer(outcome_var ~ NREM_beta + REM_beta + NREM_delta + REM_delta_mean + REM_delta_within + (1|subject),
                   REML = FALSE, data = df_sr_centered_pow)
  m_reduced = lmer(outcome_var ~ NREM_beta + REM_beta + NREM_delta + REM_delta_mean + (1|subject),
                   REML = FALSE, data = df_sr_centered_pow)
  message("Effect size (within partial R²) — REM_delta: ",
          round((sigma(m_reduced)^2 - sigma(m_full)^2) / sigma(m_reduced)^2, 4))
